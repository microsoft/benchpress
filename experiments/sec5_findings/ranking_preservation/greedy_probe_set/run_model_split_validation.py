#!/usr/bin/env python3
"""Model-split validation for ranking-preservation greedy probes.

Select probe benchmarks on training model rows using margin-5 pairwise ranking
accuracy, then validate each selected prefix on held-out model rows under the
same isolated-new-model protocol used by Section 5.1 probe validation.
"""
...
MODEL_SPLIT_PROTOCOL = "model_split_ranking_probe_validation_v1"
...
def _eval_train_candidate(args):
    probe_set, cand_j, target_by_model, base_matrix = args
...
def _cache_root(
    out_path: str,
    candidate_allowlist_ids: list[str] | None,
...
def _load_candidate_cache(
    ...
    if payload.get("eval_protocol") != MODEL_SPLIT_PROTOCOL:
...
def _validate_probe_set(
    ...
    validation_model_indices: list[int],
) -> dict[str, Any]:
    non_probe_predictions, non_probe_score_metrics, _ = (
        evaluate_probe_set_on_heldout_models(
            probe_set,
            predict_benchpress_scores,
    ...
    with_probe_predictions, with_probe_score_metrics, _ = (
        evaluate_probe_set_on_heldout_models(
            probe_set,
            predict_benchpress_scores,
            validation_model_indices,
    ...
    return {
        "non_probe": _validation_payload(non_probe_predictions, non_probe_score_metrics),
        "with_probe_zero": _validation_payload(
            with_probe_predictions,
            with_probe_score_metrics,
        ),
    }
...
def main() -> None:
    ...
    candidate_allowlist, candidate_allowlist_ids = load_benchmark_allowlist(
        args.candidate_allowlist,
        label="Candidate allowlist",
    )
    if args.out is None:
        suffix = "usercheap" if candidate_allowlist is not None else "all"
        args.out = (
            f"model_split_validation_pairwise_margin5_"
    ...
    candidates = (
        [BENCH_IDS.index(bid) for bid in candidate_allowlist_ids]
        if candidate_allowlist is not None
        else list(range(N_BENCH))
    )
    ...
    print(
        ...
        f"validation={len(validation_model_indices)} models/"
        f"{n_validation_targets} cells"
    )
    print(f"Candidates: {len(candidates)}; workers={args.workers}")
        ...
        for step in range(len(selected) + 1, args.max_steps + 1):
            ...
            for cand_j in remaining:
                ...
                futures[pool.submit(
                    ...
                    (selected + [cand_j], cand_j, train_target_by_model, train_base_matrix),
                )] = cand_j
            ...
            trajectory.append({
                ...
                "added_benchmark_name": BENCH_NAMES.get(
                    BENCH_IDS[best_j],
                    BENCH_IDS[best_j],
                ),
                "score": best_score,
                ...
                "train": {
                    ...
                    "n_correct": best_record["n_correct"],
                    "n_predicted_ties": best_record["n_predicted_ties"],
                    "pooled_accuracy": best_record["pooled_accuracy"],
                    "pooled_n_pairs": best_record["pooled_n_pairs"],
                    "pooled_n_correct": best_record["pooled_n_correct"],
            ...
            output = {
                "config": {
                    **expected_config,
                    "experiment": (
                        "sec5_findings/ranking_preservation/greedy_probe_set"
                    ),
                    "n_models": N_MODELS,
                    "n_bench": N_BENCH,
                    ...
                    "validation_model_names": _model_name_payload(
                        validation_model_indices
                    ),
                    "selection_eval_scope": (
                        "training model rows only; probe cells are exact predictions "
                    ...
                    "validation_eval_scope": (
                        ...
                        "observed probe cells as pred=true for compatibility"
                    ),
                    "cell_masking": (
                        "Validation isolates each held-out target model: the predictor "
                        "sees training rows plus that target model probe cells, and "
            ...
            val_acc = validation["non_probe"]["pairwise_accuracy_margin5"]
            print(
                f"Step {step}: added {BENCH_IDS[best_j]} "
                f"train_acc={best_record['pairwise_accuracy_margin5']:.4f} "
                f"val_non_probe_acc={val_acc:.4f} "
