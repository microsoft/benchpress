...
H4 tests whether a model row's low-rank fit predicts model-side BenchPress error. The feature is row-wise rank-2 R2.
...
```bash
cd experiments/sec6_trust/prediction_error_analysis/model_analysis
bash H4_rank2_expressibility/run.sh
```
...
Current protocol: 84 x 133 score matrix, model hide-half holdout, `N_SEEDS = 10`, `SEED = 42`, feature `rank2_R2`.
